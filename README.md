...
1. I added .devcontainer to work in a remote developmemt container with VS Code
1. The course uses a MongoDB as part of it's backend. I chose to run this in docker containers by
...
1. In his course Kevin uses Robo 3T as frontend to see the MongoDB. I prefer mongo-express, which I run by

```bash
$ docker run --rm -d --network backend --name mongo-express -p 8081:8081 -e ME_CONFIG_MONGODB_SERVER=mongodb mongo-express
```

1. Access the MongoDB frontend with `http://localhost:8081`
1. I changed the MongoDB setting in graphql-backend/src/index.js (line 16...) to
